...
- **מעבר שפה אנגלית/עברית**: לחצן שפה ליד כותרת האפליקציה מחליף את כל הממשק בין אנגלית לעברית. במעבר לעברית כל הפריסה עוברת לכיוון RTL (מימין לשמאל) — סרגל הצד, הטבלאות והטקסט כולם מתהפכים בהתאם. הנתונים שהזנת בעצמך (שמות פרויקטים, הערות וכו') לעולם לא מתורגמים.

## עדכון: פילטרים, מנוע החלטות, וטאב חדש

- **פילטרים גלובליים**: בראש ה-Dashboard נוסף סרגל פילטרים עם Project ו-Release. אפשר לצמצם את כל ה-Widgets, הטבלאות, וייצואי ה-CSV/Markdown לפרויקט ספציפי, לגרסה ספציפית, או להשאיר על "All" לתצוגת Portfolio מלאה.
- **מנוע החלטות Go/No-Go**: כשבוחרים Release בסרגל הפילטרים, המערכת מחשבת המלצה (Go / Conditional Go / No-Go / Need More Data) עם ציון (0-100) והסברים. כפתור "Generate Decision Snapshot" קופא את ההחלטה הנוכחית לתיעוד — שימושי ממש לפני פגישת Go/No-Go.
- **טאב חדש — Jira Import Center**: אפשר לייבא נתוני Jira מקובץ CSV (בלי חשבון Jira ובלי API Token) — ראה סעיף 3A למדריך המלא צעד-אחר-צעד.
...
# 3A. איך לייבא נתונים מ־Jira לפי FixVersion

זהו ייבוא מבוסס **CSV בלבד**. אין כאן שום חיבור חי ל-Jira, אין צורך בחשבון Jira, ואין צורך ב-API Token בשום שלב — מייצאים קובץ מתוך Jira ואז מעלים אותו לתוך האפליקציה. שום דבר לא נשלח בחזרה ל-Jira.

1. פתח את Jira.
2. עבור למסך חיפוש Issues.
3. הרץ JQL (עם שם הפרויקט והגרסה שלך):
   ```
   project = YOURPROJECT AND fixVersion = "YOURVERSION"
   ```
4. ייצא ל-CSV.
5. פתח את ה-Command Center (`index.html`).
6. פתח את מרכז ייבוא Jira (Jira Import Center).
7. בחר פרויקט יעד.
8. בחר גרסת יעד (Release).
9. העלה את קובץ ה-CSV.
10. בדוק את מיפוי השדות (Field Mapping) — המערכת ממפה אוטומטית עמודות סטנדרטיות של Jira (Key, Summary, Issue Type, Status, Priority, Assignee, Fix Version/s, Components וכו'), ואפשר לערוך כל מיפוי במידת הצורך.
11. צפה בתצוגה מקדימה (Preview) של השורות לפני הייבוא בפועל.
12. בצע ייבוא (Import) — לאחר בחירת Import Mode (למשל "Import Bugs as Bugs + Stories/Tasks as Release Scope").
13. בדוק את הבאגים/הסיכונים/הלוח שנוצרו אוטומטית — המערכת מייצרת הצעות לסיכונים על סמך דפוסים בנתוני Jira (למשל באג קריטי פתוח, פריטים ללא Assignee, פריטים ישנים שלא עודכנו), עם תיוג של מפתח ה-Jira הרלוונטי כראיה.
14. בצע ייצוא JSON כגיבוי, כדי לשמור גם את הנתונים שיובאו.

קובץ לדוגמה מוכן לשימוש נמצא ב-`03_DATA/samples/sample_jira_fixversion_export.csv`, עם הסברים מלאים ב-`03_DATA/samples/README_JIRA_SAMPLE_IMPORT.md` — מומלץ לנסות איתו את כל התהליך לפני ייבוא קובץ Jira אמיתי משלך.
...
4. היא לא מחוברת אוטומטית ל־Jira — אין חיבור API בזמן אמת. ניתן לייבא נתוני Jira רק דרך ייצוא CSV ידני, באמצעות ה-Jira Import Center (ראה סעיף 3A).
...
| `03_DATA/samples/sample_jira_fixversion_export.csv` | קובץ CSV לדוגמה לבדיקת Jira Import Center |
